...
def room_traversal(player, world, graph):
    ...
    while len(visited_rooms) < len(world.rooms):
        # get the top room from the stack and assign it to the current room
        current_room = s.get(-1)
        # add it to the visited rooms
        visited_rooms.add(current_room)
        # get the available directions from the current room
        neighbor_rooms = graph[current_room][1]

        # Create an empty queue to store the rooms to visit
        rooms_to_visit = Queue()
        # loop over available directions from the current room
        for neighbor_room in neighbor_rooms.items():
            # if the neighbor room has not been visited
            if neighbor_room[1] not in visited_rooms:
                # add it to the rooms to visit
                rooms_to_visit.enqueue(neighbor_room[1])

        # if there are no rooms to visit from the current room
        if rooms_to_visit.size() == 0:
            # move down in the stack
            next_room = s.get(-2)
            s.pop()

        # if there are still rooms to visit from current room
        else:
            # remove the room from the queue
            next_room = rooms_to_visit.dequeue() 
            # add it to the stack
            s.push(next_room) 

        # loop over neighbor rooms to get direction and room id
        for direction, neighbor_room_id in neighbor_rooms.items():
            # if neighbor room id match with the next room
            if neighbor_room_id == next_room:
                # add it to the path
                path.append(direction)

    return path

traversalPath = room_traversal(player, world, roomGraph)
...
# print("# This is the room", player.currentRoom.id)
# print("# Available Directions", player.currentRoom.getExits())
# print("# Let's travel North")
# player.travel('n')
# print("# We are in the north room with id ", player.currentRoom.id)
while True:
